        ...
        vol.Required(CONF_SCHEDULE): vol.Schema(
            {
                ...
                vol.Optional(CONF_STARTING_DAY): vol.All(
                    vol.Coerce(int), vol.Range(min=1, max=21)
                ),
            }
        ),
        vol.Required(CONF_SCHEDULE_OVERRIDE): vol.All(
            vol.Coerce(int), vol.Range(min=0, max=63)
        ),
        vol.Optional(CONF_MIN_TEMPERATURE): vol.All(
            vol.Coerce(int), vol.Range(min=TEMP_MIN, max=TEMP_MAX)
        ),
        vol.Optional(CONF_MAX_TEMPERATURE): vol.All(
            vol.Coerce(int), vol.Range(min=TEMP_MIN, max=TEMP_MAX)
        ),
    }
)
...
class BmrRoomClimate(ClimateEntity):
    ...
    def __init__(
        ...
        self._unique_id = (
            f"{self._bmr.getUniqueId()}-climate-{self._config.get(CONF_CIRCUIT_ID)}"
        )
    ...
    def min_temp(self):
        return (
            self._config.get(CONF_MIN_TEMPERATURE) or self._min_temperature or TEMP_MIN
        )

    @property
    def max_temp(self):
        return (
            self._config.get(CONF_MAX_TEMPERATURE) or self._max_temperature or TEMP_MAX
        )
    ...
    def hvac_mode(self):
        ...
        elif [self._config.get(CONF_SCHEDULE_OVERRIDE)] == self._schedule.get(
            "day_schedules"
        ):
            if self._can_cool:
    ...
    def set_hvac_mode(self, hvac_mode):
        ...
        if hvac_mode == HVAC_MODE_OFF:
            ...
            self._bmr.setSummerModeAssignments(
                [self._config.get(CONF_CIRCUIT_ID)], True
            )
            ...
            self._bmr.setSummerModeAssignments(
                [self._config.get(CONF_CIRCUIT_ID)], False
            )
    ...
    def update(self):
        ...
        try:
            ...
            self._schedule = self._bmr.getCircuitSchedules(
                self._config.get(CONF_CIRCUIT_ID)
            )
